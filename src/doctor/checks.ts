...
export function checkBinSymlinks(): CheckResult {
  ...
  const missing = [];
  if (!lamExists) missing.push('lam');
  if (!mcpExists) missing.push('laminar-mcp');
...
export function checkLaminarConfig(): CheckResult {
