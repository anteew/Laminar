...
function compareFingerprints(basePath: string, headPath: string): ComparisonResult {
  const baseRecords: FingerprintRecord[] = JSON.parse(fs.readFileSync(basePath, 'utf-8'));
  const headRecords: FingerprintRecord[] = JSON.parse(fs.readFileSync(headPath, 'utf-8'));
  
  const baseMap = new Map<string, FingerprintRecord>();
  const headMap = new Map<string, FingerprintRecord>();
  ...
  const unchanged: FingerprintRecord[] = [];
...
function formatMarkdownReport(result: ComparisonResult): string {
  const lines: string[] = [];
