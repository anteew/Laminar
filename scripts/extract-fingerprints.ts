...
import { generateFingerprint, extractFailureInfo, HistoryEntry } from '../src/digest/fingerprint.js';
import { DigestOutput } from '../src/digest/generator.js';
...
function extractFingerprintsFromReports(reportsDir: string): FingerprintRecord[] {
  ...
  for (const digestPath of digestFiles) {
    try {
      ...
      const digest: DigestOutput = JSON.parse(content);
      
      if (digest.status === 'fail') {
        const failureInfo = extractFailureInfo(
          digest.case,
          digest.error,
          digest.events.find(e => e.lvl === 'error')?.payload
        );
...
function findDigestFiles(dir: string): string[] {
