...
interface LinkIssue {
  ...
  line: number;
  link: string;
  type: 'broken-file' | 'broken-anchor' | 'invalid-path';
  message: string;
}
 ...
 */
function extractLinks(content: string, filePath: string): { line: number; link: string; type: 'markdown' | 'inline-path' }[] {
  const links: { line: number; link: string; type: 'markdown' | 'inline-path' }[] = [];
 ...
 */
function resolveLink(link: string, markdownFile: string, rootDir: string): string {
 ...
 */
function hasValidAnchor(link: string, targetFile: string): boolean {
  const anchorMatch = link.match(/#(.+)$/);
  if (!anchorMatch) return true; // No anchor to validate
 ...
 */
function validateLink(link: { line: number; link: string; type: string }, markdownFile: string, rootDir: string): LinkIssue | null {
