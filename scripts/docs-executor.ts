...
function parseMarkdownCodeBlocks(filePath: string): CodeBlock[] {
  ...
  let currentBlock: { start: number; code: string[]; language: string; tags: string[] } | null = null;
 ...
 */
function executeInSandbox(command: string, sandboxDir: string): { stdout: string; stderr: string; exitCode: number } {
  try {
...
async function main() {
  ...
  const runnableBlocks = allBlocks.filter(block => 
    block.language === 'bash' && block.tags.includes('runnable')
  );
